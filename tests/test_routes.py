...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        ...
        self.assertEqual(test_product.category.name, json_data['category'])
    ...
    def test_query_by_availability_false(self):
        """It should Query Products by availability false"""
        products = self._create_products(10)
        available_products = [product for product in products if product.available is False]
        available_count = len(available_products)        
        # test for available
        response = self.client.get(
